...
const ServiceOptionsToggle: React.FC<ServiceOptionsToggleProps> = ({
  serviceOptions = [],
  // Provide default empty array to prevent undefined errors
  onToggle
}) => {
  return <div className="glass-panel p-5 md:p-6 space-y-4 py-[26px] rounded-none px-[217px]">
      <h3 className="text-base md:text-lg font-semibold text-white mb-3">Additional Services</h3>
      
      <div className="space-y-3">
        {Array.isArray(serviceOptions) && serviceOptions.map(option => <div key={option.id} className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Switch checked={option.isActive || false} onCheckedChange={checked => onToggle(option.id, checked)} className={`${option.isActive ? "bg-mlbb-purple" : "bg-gray-600"}`} />
            </div>
          </div>)}
      </div>
    </div>;
