...
const formSchema = z.object({
  discordLink: z.string().url({ message: "Please enter a valid Discord invite URL" }),
  communityName: z.string().min(1, { message: "Community name is required" }),
  onlineText: z.string().min(1, { message: "Online text is required" }),
  guidelinesText: z.string().optional(),
  buttonText: z.string().min(1, { message: "Button text is required" }),
});

const DiscordLinkEditor = () => {
  const { toast } = useToast();
  const [isLoading, setIsLoading] = useState(false);

  // Get the discord settings from localStorage
  const getDiscordSettings = () => {
    const settings = localStorage.getItem("discordSettings");
    if (settings) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: getDiscordSettings(),
  });

  useEffect(() => {
    // Load the discord settings from localStorage when the component mounts
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    setIsLoading(true);
    try {
      // Save the discord settings to localStorage
      localStorage.setItem("discordSettings", JSON.stringify(values));
      
      // Dispatch an event to notify components that the discord settings have changed
      const event = new CustomEvent("discordSettingsChanged", { detail: values });
      window.dispatchEvent(event);
      
      toast({
        title: "Discord settings saved",
        description: "Your Discord community link has been updated successfully.",
      });
    } catch (error) {
      toast({
        title: "Error saving settings",
        description: "An error occurred while saving your Discord settings.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="space-y-6">
      <div className="glass-panel p-4 md:p-6">
        <h3 className="text-lg font-semibold text-white mb-4">Discord Community Settings</h3>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="https://discord.gg/your-invite-link" {...field} />
                  </FormControl>
                  <FormDescription>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Name shown on the button (e.g., "Discord")
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Online Status Text</FormLabel>
                  <FormControl>
                    <Input placeholder="42 Heroes Online" {...field} />
                  </FormControl>
                  <FormDescription>
                    Text showing how many users are online
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Guidelines Text</FormLabel>
                  <FormControl>
                    <Input placeholder="By joining, you agree to the server guidelines" {...field} />
                  </FormControl>
                  <FormDescription>
                    The small text shown below the button
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Join Now!" {...field} />
                  </FormControl>
                  <FormDescription>
            ...
            <Button 
              type="submit" 
              disabled={isLoading}
              className="w-full mt-6 bg-gradient-to-r from-mlbb-purple to-mlbb-darkpurple"
            >
              {isLoading ? "Saving..." : <><Save className="mr-2 h-4 w-4" />Save Discord Settings</>}
            </Button>
          </form>
        </Form>
        ...
        <div className="mt-6 pt-6 border-t border-white/10">
          ...
          <div className="bg-[#1a2039] p-4 rounded-lg overflow-hidden">
            <DiscordPreview 
              discordLink={form.watch("discordLink")}
              communityName={form.watch("communityName")}
              onlineText={form.watch("onlineText")}
              guidelinesText={form.watch("guidelinesText")}
...
// Preview component for the admin panel
  ...
  return (
    <div className="max-w-md mx-auto">
      <div className="flex items-center gap-2 mb-2 text-white">
        ...
        <span className="font-medium">{communityName} Community Chat</span>
      </div>
      ...
      <div className="bg-[#f6d26b] text-black rounded-lg p-3 flex flex-col">
        <div className="flex items-center">
          ...
          <div>
            <div className="font-bold text-lg">{communityName}</div>
            <div className="text-sm opacity-80">{onlineText}</div>
          </div>
        </div>
        
        <div className="flex items-center justify-between mt-3">
          <span className="text-xs opacity-70">{guidelinesText}</span>
          <a 
            href={discordLink}
            target="_blank"
            rel="noreferrer"
...
export default DiscordLinkEditor;
